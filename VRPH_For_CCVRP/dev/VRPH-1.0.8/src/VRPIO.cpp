...
void VRP::read_TSPLIB_file(const char *node_file)
{
    ...
    for(;;)
    {
		...
		&& (strncmp(temp2,"CCVRP",5)!=0) ) // M.CISSE ->before : | ) |
            {
...
//////////////////////////////////////////
// M.CISSE ->before : | |
	    if(strncmp(temp2,"EUC_2D_INT",10)==0)
            {
                edge_weight_type=VRPH_EUC_2D_INT;
            }

	    if(strncmp(temp2,"EUC_2D_DBL",10)==0)
            {
                edge_weight_type=VRPH_EUC_2D_DBL;
            }

	    if(strncmp(temp2,"EUC_2D_1DD",10)==0)
            {
                edge_weight_type=VRPH_EUC_2D_1DD;
            }
// end of add
////////////////////////////////////////////
...
// M.CISSE ->freaky : the following comparison is it useful ?
            if(strncmp(temp2,"MAN_3D",6)==0)
            {
                edge_weight_type=VRPH_MAN_3D;
        ...
        case 9:
            ...
            temp2=strtok(NULL," ");
	    // M.CISSE is it normal TWO_COORD vs TWOD_COORDS ? see a-n14-c4
            if( (strncmp(temp2,"TWOD_COORD",10)!=0) && 
		(strncmp(temp2,"TWOD_COORDS",11)!=0) && (strncmp(temp2,"THREED_COORDS",13)!=0) )
            {
	    ...
	    // M.CISSE -> the follwing lines are the key lines to support clusters
        ...
        case 25:
            ...
            break;
////////////////////////////////////////////////////
// M.CISSE : // TOUR_LENGTH and CLUSTER_SECTION 
	case 26:
	  // TOUR_LENGTH
	  // Now, we didn't meet instances with TOUR_LENGTH spec
	    break;
        case 27:
            // CLUSTER_SECTION
	  i=0;
	  ...
	  while(i<= num_nodes+1)
	    { 
	      // we get the id and the cluster number
	      fscanf(infile,"%d %d\n",&x,&y);
	      nodes[i].id=x;
	      nodes[i].cluster=y;
	      // we update the number of clusters if it is a new cluster
	      bool new_cluster_p = true;
	      for(j=0; j<i; j++){
...
/////////////////////////////////////////////////////
 ...
 down:
    ...
    int n= this->num_original_nodes;                    
    real_d = new double* [n+2];
    real_d[0] = new double [(n+2)*(n+2)];
    for(i = 1; i < n+2; i++)
      real_d[i] = real_d[i-1] + (n+2) ;
  
    for(i=0;i<=n+1;i++){
      for(j=0;j<=n+1;j++){
	if(this->d[i][j] > bigest_distance){
	  bigest_distance = this->d[i][j];
	}
      }
    }
    for(i=0;i<=n+1;i++){
      for(j=0;j<=n+1;j++){
	this->real_d[i][j] = this->d[i][j];
	if((has_cluster == true) && 
	   (this->nodes[i].cluster != this->nodes[j].cluster)){
	  this->d[i][j] = this->d[i][j] + (bigest_distance * VRPH_BIG_M);
	}
      }
    }
  }
