...
bool VRP::plot(const char *filename, int options, int orientation)
{
    ...
    unsigned int previous = VRPH_DEPOT;

#if TSPLIB_DEBUG
    int clusters_seen[num_clusters+1]; 
    if (has_cluster == true){
      for(i=0;i<= num_clusters+1;i++){
	clusters_seen[i] = 0; // initialised to false
      }
    }
    int current_cluster= 0; // we are in the cluster of the depot;
    bool print_consistency = true;
#endif
...
#if TSPLIB_DEBUG
	if (has_cluster == true) {
	  // At the begenning, we are in the depot cluster
	  current_cluster = nodes[VRPH_DEPOT].cluster;
	}
#endif
        ...
        {
#if TSPLIB_DEBUG
	  if ((has_cluster == true) && (print_consistency == true)){
	    if(nodes[current].cluster != current_cluster){
	      if(clusters_seen[nodes[current].cluster] !=0){
		printf("error :\n");
		printf("=========================================================");
		printf("the solution found is not consistent for the cluster %d\n", nodes[current].cluster);
		printf("=========================================================");
		cluster_success = 1;
		print_consistency = false;
	      }
	      else{
		clusters_seen[nodes[current].cluster] = 1; // mark the cluster as seen
		current_cluster = nodes[current].cluster;
	      }
	    }
	  }
#endif
            ...
            ctr++;    
	    if (has_cluster == true){
	      current_distance += real_d[current][previous];
	      previous = current;
	    }
        ...
        {
	  if (has_cluster == true){
	    current_distance += real_d[previous][VRPH_DEPOT];
	  }
            x[ctr]= nodes[VRPH_DEPOT].x;
            y[ctr]= nodes[VRPH_DEPOT].y;
            ctr++;
    ...
    if (has_cluster == true)
      ...
      sprintf(dest,"%d nodes   %7.2f    %d routes   %d clusters", num_nodes, current_distance,
	      count_num_routes(), num_clusters);
    ...
    for(i=1;i<=this->num_original_nodes;i++)
    {
		...
		if(has_cluster == true) // So, node of the same cluster will take the same color
		  plcol0(this->nodes[i].cluster % VRPH_MAX_COLOR);
                plpoin(1,x+j,y+j,4);
