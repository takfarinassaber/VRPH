...
double VRPDistance(int type, double x1, double y1, double x2, double y2)
{
	...
	// M.CISSE ///////////////////////////////////////////////
	case VRPH_EUC_2D_INT:
	  xd = x1-x2;
	  yd = y1-y2;
	  return floor(sqrt(xd*xd + yd*yd)); // M.CISSE : hum, is nint is really floor or ceil ?

	case VRPH_EUC_2D_DBL:
	  xd = x1-x2;
	  yd = y1-y2;
	  return sqrt(xd*xd + yd*yd);

	case VRPH_EUC_2D_1DD:
	  xd = x1-x2;
	  yd = y1-y2;
	  return 1/10 * floor(10*sqrt(xd*xd + yd*yd));
