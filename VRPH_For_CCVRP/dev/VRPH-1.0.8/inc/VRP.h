...
public:
    ...
    // Clusters
    bool get_has_cluster();
    int get_cluster_success();
...
private:  
    ...
    int cluster_success;
    bool has_cluster;
    unsigned int num_clusters;   // 1 + num_cluster_of_node because depot is a (one,1) cluster
    double bigest_distance;       // the distance between the farthest node
    double **real_d;       // The real distance matrix d because the distance matrix d is penalize by VRPH_BIG_M
